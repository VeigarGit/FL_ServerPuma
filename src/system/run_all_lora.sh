...
# =============================================================================
# run_all_lora.sh — Executa a sequência de simulações LoRA Padrão nos 4 datasets
#
# Configuração LoRA Padrão (Baseline):
#   - Estratégia: lora
#   - Pruning: Sem poda
#   - PaCA: Fixo em 12 camadas (todas as camadas do CLIP, sem adaptive-paca)
#   - Modelo: CLIP
#   - Rodadas: 150
#   - Clientes: 25
...
#   tmux new -s orquestrador_lora
#   cd /home/rafael.teixeira.silva/David/FL_ServerPuma/src/system
#   ./run_all_lora.sh [opções como -did 1, --simulations 10, etc.]
# =============================================================================
...
if [ -n "$TMUX" ]; then
    ...
    if [ "$CURRENT_TMUX_SESSION" = "fl_puma" ]; then
        ...
        echo "  tmux rename-session -t fl_puma orquestrador_lora"
        echo "ou inicie uma nova com: tmux new -s orquestrador_lora"
        exit 1
    fi
fi

# Parâmetros padrão do LoRA
SIMULATIONS=10
CLIENTS=25
ROUNDS=150
STRATEGY="lora"
...
show_help() {
    cat << 'EOF'
Uso: ./run_all_lora.sh [opções]

Executa automaticamente as 10 simulações de LoRA Padrão (Baseline)
(sem pruning, sem adaptive-paca, PaCA fixo em 12 camadas)
...
echo "================================================================="
echo "        ORQUESTRADOR DE EXPERIMENTOS LoRA PADRÃO (CLIP)"
echo "================================================================="
echo "  Estratégia:         $STRATEGY (sem poda)"
echo "  PaCA:               FIXO em 12 (sem adaptive-paca)"
echo "  Modelo:             $MODEL"
...
for ENTRY in "${DATASETS[@]}"; do
    ...
    echo "================================================================="
    echo "[$IDX/$TOTAL_DATASETS] Iniciando LoRA: $DATASET_NAME ($NUM_CLASSES classes)"
...
echo "🎉 TODOS OS EXPERIMENTOS LoRA FORAM CONCLUÍDOS COM SUCESSO!"
